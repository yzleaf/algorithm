...
public class Hash {
    ...
    public class RehashingSolution {
        public ListNode[] rehashing(ListNode[] hashTable) {
            ...
            for (int i = 0; i < hashTable.length; i++) { // 遍历原始hash数组

                while (hashTable[i] != null) { // 这个hash链表数组的某一个位置有数据
                    ...
                    if (newHashTable[newIndex] == null) {
                        ...
                        ListNode curr = newHashTable[newIndex];
                        while (curr.next != null) {
                            curr = curr.next;
                        }
                        curr.next = new ListNode(hashTable[i].val);
                    }
